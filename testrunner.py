...
defaults = {'jalangi': './jalangi2', 'sherlock': './sherlock.js', 'tests': './tests'}
	...
	if not test:
		...
		if check_extension(file, 'json'):			
			with open(os.path.join(d['tests'], file), 'r') as result_file:
				...
				command = 'node ' + d['jalangi'] + '/src/js/commands/jalangi.js ' +\
						  '--inlineIID --inlineSource --analysis ' + \
						  d['jalangi'] + '/src/js/sample_analyses/ChainedAnalyses.js ' + \
